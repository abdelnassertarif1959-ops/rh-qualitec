<template>
  <div class="space-y-6">
    ...
    <div class="min-h-[400px]">
      ...
      <!-- Aba: Documentos (apenas edição) -->
      <div v-if="abaAtiva === 'documentos' && isEditing && form.id">
        <FuncionarioDocumentos :funcionario-id="form.id" />
      </div>
      ...
      <div v-if="abaAtiva === 'beneficios'" class="space-y-6">
        ...
        <!-- Pensão Alimentícia - Apenas para CLT -->
        <div v-if="form.tipo_contrato !== 'PJ'" class="p-4 border border-gray-200 rounded-xl space-y-4">
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              <svg class="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
              </svg>
              <h5 class="font-semibold text-gray-800">Pensão Alimentícia</h5>
            </div>
            <UiCheckbox 
              v-model="form.pensao_config_ativa" 
              label="Ativa"
            />
          </div>
          
          <div v-if="form.pensao_config_ativa" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <UiSelect 
              v-model="form.pensao_config_tipo" 
              :options="[
            ...
            <UiInput 
              v-if="form.pensao_config_tipo === 'percentual'"
              v-model="form.pensao_config_percentual" 
              type="number" 
              step="0.01"
              label="Percentual (%)" 
              placeholder="30.00"
            ...
            <UiInput 
              v-if="form.pensao_config_tipo === 'fixo'"
              v-model="form.pensao_config_valor_fixo" 
              type="number" 
              step="0.01"
              label="Valor Fixo (R$)" 
              placeholder="0,00"
...
import FuncionarioDocumentos from './FuncionarioDocumentos.vue'
...
const inicializarBeneficios = () => {
  ...
  // Inicializar configuração de pensão se não existir
  if (props.form.pensao_config_ativa === undefined) {
    props.form.pensao_config_ativa = false
  }
  if (!props.form.pensao_config_tipo) {
    props.form.pensao_config_tipo = 'percentual'
  }
  if (props.form.pensao_config_percentual === undefined) {
    props.form.pensao_config_percentual = 0
  }
  if (props.form.pensao_config_valor_fixo === undefined) {
    props.form.pensao_config_valor_fixo = 0
  }
  if (props.form.pensao_config_recorrente === undefined) {
    props.form.pensao_config_recorrente = true
  }
...
const calcularTotalDescontos = () => {
  ...
  // Pensão Alimentícia
  if (props.form.pensao_config_ativa) {
    if (props.form.pensao_config_tipo === 'fixo') {
      total += parseFloat(props.form.pensao_config_valor_fixo) || 0
    } else {
      const pct = parseFloat(props.form.pensao_config_percentual) || 0
      total += (salarioBase * pct) / 100
    }
  }
